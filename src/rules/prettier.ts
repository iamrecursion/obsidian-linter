...
export default class Prettier extends RuleBuilder<PrettierOptions> {
  ...
  getPrettierOptions(options: PrettierOptions): FormatterOptions {
    ...
    return {
      ...
      __languageMappings,
      // @ts-expect-error
      useTabs: options['use-tabs'], // eslint-disable-line @typescript-eslint/no-unsafe-assignment -- the @ts-expect-error above leaves this value error-typed
      // @ts-expect-error
      proseWrap: proseStyleToPrettierString(options['prose-style']), // eslint-disable-line @typescript-eslint/no-unsafe-argument -- the @ts-expect-error above leaves this value error-typed
      // @ts-expect-error
  ...
  async runPrettier(text: string, options: PrettierOptions): Promise<string> {
    const mungedOptions = this.buildRuleOptions(options);
